...
namespace Ace_Super_Market.CapaLogica
{
    class crearTiqueteElectronico
    {
                                      ...
                                      List<String> nombreProducto, List<String> unidadMedida)
        {
            ...
            CapaLogica.consultaHacienda objCH = new consultaHacienda();
            CapaLogica.generarPDFs objGPDF = new generarPDFs();
            CapaLogica.envioCorreo objEC = new envioCorreo();
            String XMLFirmado = objFD.firmadoTiqueteElectronico(pathFactura, Dclave["consecutivo"]);

            String XMLCodificado = objCOD.XMLtoBase64(XMLFirmado);

            objEH.envioTiqueteXML(XMLCodificado, objG.optenerToken(), Clave, thisDay.ToString("yyyy-MM-ddTHH:mm:ss"));

            Boolean result = objCH.verificacionEstatus(Clave, objG.optenerToken(), Dclave["consecutivo"]);
            ...
            String FacturaFirmada = @"C:\Program Files (x86)\GPD Soluciones\Ace Super Market\XML\XMLFirmados\TiqueteElectronicoCF" + Dclave["consecutivo"] + ".xml";


            String pathPDF = objGPDF.generarPDF(Clave, Dclave["consecutivo"], Dcliente, precioProducto, cantidad, total, nombreProducto, tipoProducto, Dclave["codigoseguridad"], "Tiquete");
